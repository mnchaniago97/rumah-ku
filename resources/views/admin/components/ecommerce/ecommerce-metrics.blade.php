...
@php
  $defaultCards = [
    ['label' => 'Total Properti', 'value' => 0, 'change_pct' => 0, 'positive' => true],
    ['label' => 'Pending Approval', 'value' => 0, 'change_pct' => 0, 'positive' => true],
  ];

  $cardsList = count($cards) ? $cards : $defaultCards;

  $first = $cardsList[0] ?? $defaultCards[0];
  $second = $cardsList[1] ?? $defaultCards[1];
...
<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-6">
    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
      ...
      <div class="flex items-end justify-between mt-5">
        ...
        @php $firstPositive = (bool)($first['positive'] ?? true); @endphp
        <span class="{{ $badgeClasses($firstPositive) }}">
          <svg
            class="fill-current"
            width="12"
            height="12"
          ...
          {{ number_format((float)($first['change_pct'] ?? 0), 2, '.', '') }}%
        </span>
      </div>
    </div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
      ...
      <div class="flex items-end justify-between mt-5">
        ...
        @php $secondPositive = (bool)($second['positive'] ?? true); @endphp
        <span class="{{ $badgeClasses($secondPositive) }}">
          <svg
            class="fill-current"
            width="12"
            height="12"
          ...
          {{ number_format((float)($second['change_pct'] ?? 0), 2, '.', '') }}%
        </span>
      </div>
    </div>
  </div>

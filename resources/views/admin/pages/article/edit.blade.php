...
@section('content')
    <div class="w-full">
        <div class="mb-6">
            ...
            <p class="text-gray-600">Perbarui artikel/blog</p>
        </div>
            ...
            <form action="{{ route('admin.articles.update', $article->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    <div class="col-span-2">
                        ...
                        <input type="text" name="title" value="{{ old('title', $article->title) }}" placeholder="Judul artikel"
                    ...
                    <div class="col-span-2">
                        ...
                        <textarea name="content" rows="10"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">{{ old('content', $article->content) }}</textarea>
                        @error('content')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="is_published"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
                            <option value="1" {{ (string) old('is_published', (int) $article->is_published) === '1' ? 'selected' : '' }}>
                                Published</option>
                            <option value="0" {{ (string) old('is_published', (int) $article->is_published) === '0' ? 'selected' : '' }}>
                                Draft</option>
                        </select>
                        @error('is_published')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    ...
                    <div class="col-span-2">
                        ...
                        @if ($article->image)
                            <div class="mb-3">
                                <img src="{{ Storage::url($article->image) }}" alt="Gambar" class="h-28 rounded-lg object-cover">
                            </div>
                        @endif

...
@section('content')
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Edit Post Forum</h1>
                <p class="text-sm text-gray-600 dark:text-gray-300">Perbarui konten posting forum</p>
            </div>
        ...
        <form action="{{ route('admin.forum-posts.update', $post->id) }}" method="POST"
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-12">
                <div class="md:col-span-4">
                    ...
                    <select name="category" required
                        ...
                        @foreach ($categories as $cat)
                            <option value="{{ $cat }}" @selected(old('category', $post->category) === $cat)>
                ...
                <div class="md:col-span-12">
                    ...
                    <textarea name="body" rows="8"
                        class="w-full rounded-lg border border-gray-200 bg-transparent px-4 py-3 text-sm text-gray-800 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-800 dark:text-white">{{ old('body', $post->body) }}</textarea>
                    @error('body')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mt-6 flex items-center gap-3">
                ...
                <a href="{{ route('admin.forum-posts.show', $post->id) }}" class="text-sm text-gray-600 hover:underline dark:text-gray-300">Batal</a>
            </div>
        </form>
    </div>
@endsection

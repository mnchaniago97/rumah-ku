...
@section('content')
    <div class="space-y-6">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Aset Turun Harga</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400">Kelola listing properti turun harga.</p>
            </div>
            <a href="{{ route('admin.discounted.create') }}"
                class="inline-flex items-center gap-2 rounded-lg bg-red-600 px-4 py-2 text-sm font-semibold text-white shadow-theme-xs hover:bg-red-700">
                <i class="fas fa-plus"></i> Tambah Aset Turun Harga
            </a>
        </div>
        ...
        <div class="grid w-full grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                ...
                <div class="relative w-full">
                    <div class="relative inline-block w-full transform transition-transform duration-300 ease-in-out hover:-translate-y-2">
                        <div class="rounded-lg bg-white p-4 shadow">
                            <a href="{{ route('admin.properties.show', $property->id ?? $property->getKey()) }}"
                                ...
                                <div class="relative flex h-52 justify-center overflow-hidden rounded-lg">
                                    ...
                                    @if($discountPercentage)
                                        <span class="absolute top-0 right-2 z-10 mt-3 ml-3 inline-flex select-none rounded-sm bg-red-600 px-2 py-1 text-xs font-semibold text-white">
                                            -{{ $discountPercentage }}%
                                        </span>
                                    @endif
                                </div>
                                ...
                                <div class="mt-4">
                                    ...
                                    <div class="mt-2">
                                        <p class="text-gray-400 line-through text-sm">
                                            <span class="text-xs uppercase">Rp</span>
                                            {{ number_format($property->original_price ?? $property->price, 0, ',', '.') }}
                                        </p>
                                        <p class="text-primary mt-1 inline-block whitespace-nowrap rounded-xl font-semibold leading-tight">
                                            <span class="text-sm uppercase">Rp</span>
                                            <span class="text-2xl">{{ number_format($property->price, 0, ',', '.') }}</span>
                                        </p>
                                    </div>
                                </div>
                                <div class="mt-4">
                            ...
                            <div class="mt-6 flex gap-2">
                                <a href="{{ route('admin.discounted.edit', $property->id ?? $property->getKey()) }}"
                                    class="flex-1 rounded-lg bg-blue-600 px-3 py-2 text-center text-sm font-semibold text-white hover:bg-blue-700">
                                    <i class="fa fa-edit mr-1"></i> Edit Diskon
                                </a>
                                <form action="{{ route('admin.discounted.destroy', $property->id ?? $property->getKey()) }}" method="POST" class="flex-1">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        class="w-full rounded-lg bg-red-600 px-3 py-2 text-sm font-semibold text-white hover:bg-red-700"
                                        onclick="return confirm('Apakah Anda yakin ingin menghapus status turun harga dari properti ini?')">
            ...
            @empty
                <div class="col-span-full rounded-xl border border-dashed border-gray-200 bg-white p-6 text-center text-sm text-gray-500 dark:border-gray-800 dark:bg-white/[0.03] dark:text-gray-400">
                    <i class="fa fa-tags text-4xl text-gray-300 mb-3"></i>
                    <p>Belum ada properti turun harga.</p>
                    <p class="mt-2 text-xs">Properti akan otomatis masuk ke sini ketika harga diturunkan.</p>
                    <a href="{{ route('admin.discounted.create') }}" class="inline-block mt-3 text-blue-600 hover:underline">
                        Tambah Aset Turun Harga
                    </a>
                </div>
            @endforelse
        </div>

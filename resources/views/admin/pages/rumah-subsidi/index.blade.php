...
@section('content')
    <div class="space-y-6">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">Kelola listing rumah subsidi.</p>
            </div>
        ...
        <div id="card-view" class="grid w-full grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                ...
                <div class="relative w-full">
                    <div class="relative inline-block w-full transform transition-transform duration-300 ease-in-out hover:-translate-y-2">
                        <div class="rounded-lg bg-white p-4 shadow">
                            <a href="{{ route('admin.rumah-subsidi.show', $property->id ?? $property->getKey()) }}"
                            ...
                            <div class="mt-6 flex gap-2">
                                <a href="{{ route('admin.rumah-subsidi.edit', $property->id ?? $property->getKey()) }}"
                                    class="flex-1 rounded-lg bg-blue-600 px-3 py-2 text-center text-sm font-semibold text-white hover:bg-blue-700">
                                    <i class="fa fa-edit mr-1"></i> Edit
                                </a>
                                <form action="{{ route('admin.rumah-subsidi.destroy', $property->id ?? $property->getKey()) }}" method="POST" class="flex-1">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        class="w-full rounded-lg bg-red-600 px-3 py-2 text-sm font-semibold text-white hover:bg-red-700"
                                        onclick="return confirm('Apakah Anda yakin ingin menghapus rumah subsidi ini?')">
            ...
            @empty
                <div class="col-span-full rounded-xl border border-dashed border-gray-200 bg-white p-6 text-center text-sm text-gray-500 dark:border-gray-800 dark:bg-white/[0.03] dark:text-gray-400">
                    ...
                    <p>Belum ada rumah subsidi.</p>
                    <a href="{{ route('admin.rumah-subsidi.create') }}" class="inline-block mt-3 text-blue-600 hover:underline">
                        Tambah Rumah Subsidi Pertama
                    </a>
                </div>
            @endforelse
        </div>
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4">
                                        @if($property->user && $property->user->role === 'admin')
                                            <span class="text-sm font-semibold text-brand-600">Official Rumah IO</span>
                                        @else
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="inline-flex items-center gap-3">
                                            <a href="{{ route('admin.rumah-subsidi.show', $property) }}" class="text-sm font-medium text-brand-600 hover:underline">Detail</a>
                                            <a href="{{ route('admin.rumah-subsidi.edit', $property) }}" class="text-sm font-medium text-blue-600 hover:underline">Edit</a>
                                            <form action="{{ route('admin.rumah-subsidi.destroy', $property) }}" method="POST" onsubmit="return confirm('Hapus properti ini?')">
                                                @csrf
                                                @method('DELETE')
    ...
    <script>
        function setView(view) {
            ...
            localStorage.setItem('rumahSubsidiViewPreference', view);
        }
        
        // Load saved preference
        document.addEventListener('DOMContentLoaded', function() {
            const savedView = localStorage.getItem('rumahSubsidiViewPreference') || 'card';
            setView(savedView);
        });
    </script>
@endsection
